                ...
                Hook { callFrame ->
                    try {
                        ...
                        val textReact = binding.a.findViewById<TextView>(textReactId).apply {
                            visibility =
                                if ((callFrame.args[0] as WidgetChatListActions.Model).manageMessageContext.canAddReactions) View.VISIBLE else View.GONE
                        }

                        if (!textReact.hasOnClickListeners()) textReact.setOnClickListener {
                            try {
                                val fragmentManager =
                                    (callFrame.thisObject as WidgetChatListActions).parentFragmentManager
                                ...
                                inDialog.setOnOkListener {
                                    val result = TextToEmoji.generateEmojiArray(inDialog.input.toString().trim())
                                    inDialog.dismiss()
